...
function renderBeds() {
    ...
    let totalAll = 0, totalHealthy = 0, totalSick = 0, totalCritical = 0, totalDead = 0, totalReplanted = 0;
    ...
    farmData.forEach((bed, bIndex) => {
        ...
        nodes.forEach((node, pIndex) => {
            ...
            if (currentFilter === 'R' && !node.replanted_date) return;
            ...
            if (state !== 'D' && node.logs && node.logs.length > 0) {
                for (let i = node.logs.length - 1; i >= 0; i--) {
                    ...
                    if (node.logs[i].replanted === true) {
                        // Stop looking further back if we hit a replant event
                        break;
                    }
                }
            }
    ...
    if (document.getElementById('countReplanted')) document.getElementById('countReplanted').innerText = totalReplanted;
...
function openBottomSheet(bed, node, bIndex, pIndex) {
    ...
    const dateTextEl = document.getElementById('bsPlantingDateText');
    const dateInputEl = document.getElementById('bsPlantingDateInput');
    
    let isReplanted = !!node.replanted_date;

    // Derive default planting date to match Land Details exactly
    let defaultPlantingDateStr = (globalCropData && globalCropData.crop && globalCropData.crop.planted_date) || '';
    if (globalCropData && globalCropData.crop) {
        try {
            const tasks = JSON.parse(globalCropData.crop.tasks_state_json || '[]');
            const zeroTask = tasks.find(t => parseInt(t.day_offset) === 0);
            if (zeroTask && zeroTask.due_date) {
                defaultPlantingDateStr = zeroTask.due_date;
            } else {
                const fallbackTask = tasks.find(t => t.title.includes('রোপণ') || t.title.includes('বপন'));
                if (fallbackTask && fallbackTask.due_date) {
                    defaultPlantingDateStr = fallbackTask.due_date;
                }
            }
        } catch (e) {}
    }
    if (defaultPlantingDateStr && defaultPlantingDateStr.includes('T')) {
        defaultPlantingDateStr = defaultPlantingDateStr.split('T')[0];
    }
    
    let displayDateStr = node.replanted_date || node.planting_date || defaultPlantingDateStr;
    
    // Allow editing for all plants using custom calendar
    const containerEl = document.getElementById('bsPlantingDateContainer');
    containerEl.style.cursor = 'pointer';
    containerEl.onclick = () => {
        const input = document.getElementById('bsPlantingDateInput');
        input.dataset.value = displayDateStr || new Date().toISOString().split('T')[0];
        if (window.openDatePicker) {
            window.openDatePicker('bsPlantingDateInput');
        } else {
    ...
    if (displayDateStr) {
        const dObj = new Date(displayDateStr);
        if (!isNaN(dObj)) {
...
window.updatePlantingDate = async function(newDate) {
    if(!currentlyEditingPlant || !newDate) return;
    const { node, bed, bIndex, pIndex } = currentlyEditingPlant;
    const isReplanted = !!node.replanted_date;
    
    if (isReplanted) {
        node.replanted_date = newDate;
    } else {
        node.planting_date = newDate;
    }

    // Auto save the new date
    ...
    try {
        ...
        console.error("Failed to update planting date", e);
    }
...
function populateTimeline(logs) {
    ...
    if (logsWithImages.length >= 2) {
        ...
        const logsArrayStr = encodeURIComponent(JSON.stringify(logsWithImages));
        ...
        <div style="margin-bottom: 20px; background:#F8FAFC; border:1px solid #E2E8F0; border-radius:12px; padding:16px; text-align:center; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
            <div style="margin-bottom: 12px; display: flex; align-items: center; justify-content: center; gap: 8px;">
                <div style="width: 40px; height: 40px; background: rgba(16, 185, 129, 0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: var(--primary-color, #10b981);">
            ...
            <button class="btn-primary w-full" style="background: var(--primary-color, #10b981); box-shadow: 0 4px 10px rgba(16, 185, 129, 0.25);" onclick="window.openImageCompare('${logsArrayStr}')">
...
window.openBatchUpdateModal = function () {
    ...
    const batchPlantingDateInput = document.getElementById('batchPlantingDateInput');
    if (batchPlantingDateInput) {
        batchPlantingDateInput.value = '';
        delete batchPlantingDateInput.dataset.value;
    }
    ...
    let hasDeadPlants = false;
    let hasHealthyPlants = false;
    selectedPlants.forEach(plantKey => {
        if (plantKey && plantKey.node) {
            if (plantKey.node.state === 'D') hasDeadPlants = true;
            else hasHealthyPlants = true;
        }
    });

    if (hasDeadPlants) {
        document.getElementById('batchReplantContainer').style.display = 'block';

        const dateInput = document.getElementById('batchReplantDate');
        const today = new Date();
        const EN_TO_BN_MONTHS = ['জানুয়ারি', 'ফেব্রুয়ারি', 'মার্চ', 'এপ্রিল', 'মে', 'জুন', 'জুলাই', 'আগস্ট', 'সেপ্টেম্বর', 'অক্টোবর', 'নভেম্বর', 'ডিসেম্বর'];
        const toBngDigits = (num) => String(num).replace(/[0-9]/g, d => '০১২৩৪৫৬৭৮৯'[d]);
        const todayStr = today.toISOString().split('T')[0];
        dateInput.dataset.value = todayStr;
        dateInput.value = `${toBngDigits(today.getDate())} ${EN_TO_BN_MONTHS[today.getMonth()]} ${toBngDigits(today.getFullYear())}`;

        document.getElementById('batchReplantVariety').value = '';
    } else {
        document.getElementById('batchReplantContainer').style.display = 'none';
    }

    if (hasDeadPlants && !hasHealthyPlants) {
        document.getElementById('batchNormalFieldsContainer').style.display = 'none';
    } else {
        document.getElementById('batchNormalFieldsContainer').style.display = 'block';
    }
...
window.saveBatchUpdateDetails = async function () {
    ...
    const batchPlantingDateInput = document.getElementById('batchPlantingDateInput');
    const bPlantingDate = batchPlantingDateInput ? batchPlantingDateInput.dataset.value : null;

    const isReplantVisible = document.getElementById('batchReplantContainer').style.display !== 'none';
    const batchReplantDateEl = document.getElementById('batchReplantDate');
    const batchReplantDate = isReplantVisible ? (batchReplantDateEl.dataset.value || batchReplantDateEl.value) : null;
    const batchReplantVariety = isReplantVisible ? document.getElementById('batchReplantVariety').value : null;
    ...
    selectedPlants.forEach(sel => {
        ...
        if (bPlantingDate) {
            if (node.replanted_date) {
                node.replanted_date = bPlantingDate;
            } else {
                node.planting_date = bPlantingDate;
            }
        }
        ...
        let hasNormalUpdate = (bLeaves || bFruits || fertActive || pestActive || bNote.trim() !== '' || bPlantingDate);

        if (isReplantVisible && node.state === 'D' && batchReplantDate) {
            node.state = 'H';
            node.replanted_date = batchReplantDate;
            if (batchReplantVariety) {
                node.variety = batchReplantVariety;
            }
            node.logs.push({
                date: batchReplantDate,
                state: 'H',
                note: `নতুন চারা রোপণ করা হয়েছে (Batch Replanted) - ${batchReplantVariety || ''}`,
                replanted: true
            });
        }
...
// ========================
let globalCompareLogs = [];

window.openImageCompare = function (encodedLogsParams) {
    try {
        const logsWithImages = JSON.parse(decodeURIComponent(encodedLogsParams));
        if (!logsWithImages || logsWithImages.length < 2) return;

        globalCompareLogs = logsWithImages;

        const newLog = logsWithImages[logsWithImages.length - 1]; // Latest
        const previousLogIndex = logsWithImages.length - 2; // Just before the latest
        const oldLog = logsWithImages[previousLogIndex];

        document.getElementById('compareImgAfter').src = newLog.image_url;
        ...
        console.error("Failed to open image comparison", e);
    }
};
...
window.confirmDateSelection = function () {
    if (window.currentCalendarTargetId && window.selectedCalendarDateStr) {
        ...
        if (targetEl) {
            ...
            const formattedBngDate = `${toBngDigits(dObj.getDate())} ${EN_TO_BN_MONTHS[dObj.getMonth()]} ${toBngDigits(dObj.getFullYear())}`;
            
            if (targetEl.tagName === 'INPUT' && targetEl.type === 'date') {
                targetEl.value = window.selectedCalendarDateStr; // yyyy-MM-dd
            } else {
                if (targetEl.value !== undefined) targetEl.value = formattedBngDate;
                else targetEl.innerText = formattedBngDate;
            }
            
            targetEl.dataset.value = window.selectedCalendarDateStr;
            
            // Trigger save when updating date from plant profile bottom sheet
            if (window.currentCalendarTargetId === 'bsPlantingDateInput' && window.updatePlantingDate) {
                window.updatePlantingDate(window.selectedCalendarDateStr);
            }
        }
    }
...
window.executeRelocation = function (type) {
    ...
    if (type === 'shift_insert') {
        ...
        const token = localStorage.getItem('farmer_jwt');
        fetch(`${API_BASE_URL}/api/crops/${cropId}/beds/${bed.id}/insert-shift`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
            },
            body: JSON.stringify({ targetPlantId: node.id })
        }).then(res => res.json()).then(data => {
            if (data.success) {
                Object.assign(bed, { plants_nodes_json: data.plants });
                renderBeds();
            } else {
                showConfirmModal('সতর্কতা', data.error || 'শিফট করতে সমস্যা হচ্ছে।', null);
            }
        }).catch(e => {
...
window.deleteShiftPlantSerial = function () {
    if (!currentlyEditingPlant) return;
    showConfirmModal('সতর্কতা', 'আপনি কি নিশ্চিত যে আপনি এই চারা বা গর্তটি মুছে ফেলতে চান? এটি মুছে ফেললে ডানদিকের সব গর্ত বামদিকে সরে আসবে এবং এই গর্তের পুরনো ছবি ডিলিট হয়ে যাবে।', async () => {
        const { bed, node, pIndex } = currentlyEditingPlant;
        closeBottomSheet();
        ...
        const token = localStorage.getItem('farmer_jwt');
        try {
            const res = await fetch(`${API_BASE_URL}/api/crops/${cropId}/beds/${bed.id}/delete-shift`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
            ...
            if (data.success) {
                ...
                showConfirmModal('সতর্কতা', data.error || 'ডিলিট করতে সমস্যা হচ্ছে।', null);
            }
        } catch (e) {
            console.error("Delete Shift API error: ", e);
            showConfirmModal('সতর্কতা', 'কোথাও সমস্যা হচ্ছে, আবার চেষ্টা করুন।', null);
        } finally {
            if (loader) loader.style.display = 'none';
        }
